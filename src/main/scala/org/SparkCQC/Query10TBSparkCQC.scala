...
object Query10TBSparkCQC {
  def main(args: Array[String]): Unit = {
    ...
    val g2l = g2.filter(x => x._2(2).asInstanceOf[Int] <= sqrtn).map(x => (x._2(0).asInstanceOf[Int], x._2))
    val g2r = g2.filter(x => x._2(2).asInstanceOf[Int] > sqrtn)
    ...
    val enum5 = C enumerationWithAnnotation (enum4, g1Annotation, Array(0, 1, 2), Array(1), 3, 0, 0, smaller, smaller)
    // result2 Schema (g3.SRC, g3.DST), annotation
    val result2=enum5.map(x=> ((x._2(1).asInstanceOf[Int], x._2(2).asInstanceOf[Int]), x._2(3).asInstanceOf[Int]))

    val result = result1.union(result2).reduceByKey((x, y) => x+y)
